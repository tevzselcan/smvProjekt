...
export const hash = async (data: string, salt = 10): Promise<string> => {
  try {
    ...
    Logging.error(error)
    throw new InternalServerErrorException('Something while hashing password.')
  }
}
...
export const compareHash = async (data: string | Buffer, encryptedData: string): Promise<boolean> => {
  try {
    ...
    Logging.error(error)
    throw new InternalServerErrorException('Something went wrong whilw comparing hash.')
  }
}
