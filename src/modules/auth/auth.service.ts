...
export class AuthService {
  ...
  async login(userFromRequest: User, res: Response): Promise<void> {
    ...
    const accessTokenCookie = await this.generateCookie(
      accessToken,
    ...
    const refreshTokenCookie = await this.generateCookie(
      refreshToken,
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'Something went wrong while setting cookies into response header. 1',
      );
    }
  }
  ...
  async signout(userId: string, res: Response): Promise<void> {
    ...
    try {
      ...
      Logging.error(error);
      throw new InternalServerErrorException(
        'Something went wrong while setting cookies into response header 2',
      );
    }
  }
  ...
  async refreshTokens(req: Request): Promise<User> {
    ...
    try {
      ...
      Logging.error(error);
      throw new InternalServerErrorException(
        'Something went wrong while setting cookies into response header 3',
      );
    }
    return user;
  }
  ...
  async getUserIfRefreshTokenMatches(
    ...
    const user = await this.userService.findById(userId);
    const isRefreshTokenMatching = await compareHash(
      refreshToken,
      user.refreshToken,
    );
